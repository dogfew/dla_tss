...
class SharedEncoder(nn.Module):
    def __init__(self, N, L1, L2, L3):
        ...
        self.L2 = L2
        self.L3 = L3
        self.encoder_1d_short = nn.Sequential(nn.Conv1d(1, N, L1, stride=L1 // 2), nn.ReLU())
        self.encoder_1d_middle = nn.Sequential(nn.Conv1d(1, N, L2, stride=L1 // 2), nn.ReLU())
        self.encoder_1d_long = nn.Sequential(nn.Conv1d(1, N, L3, stride=L1 // 2), nn.ReLU())
...
class StackedTCNs(nn.Module, ):
    def __init__(self,
        ...
        super().__init__()
        self.speaker_tcn = TCNBlockSpeaker(embedd_dim=embedd_dim,
        ...
        self.stacked_tcns = nn.Sequential(*[
            TCNBlock(in_channels=in_channels,
                     conv_channels=conv_channels,
                     kernel_size=kernel_size, dilation=(2 ** b))
...
class SpExPlus(nn.Module):
    def __init__(self,
        ...
        self.tcns = nn.ModuleList(
            [StackedTCNs(embedd_dim=embed_dim, in_channels=O,
                         conv_channels=P, kernel_size=Q,
                         num_blocks=B)
             for _ in range(R)]
        )
    ...
    def forward(self, mix_audio, reference_audio, reference_audio_len, **batch):
        ...
        # Mean Pooling
        v = x.sum(-1) / (((reference_audio_len - self.L1) // (self.L1 // 2) + 1) // 27).unsqueeze(1)
        ...
        return {'s1': s1,
                's2': s2,
                's3': s3,
                'speaker_pred': self.speaker_linear(v)}

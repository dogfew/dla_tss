...
class Audio():
    def __init__(self,
                 ...
                 min_level=-100):
        self.n_fft = n_fft
        self.hop_length = hop_length if hop_length is not None else math.floor(n_fft / 4)
        self.win_length = win_length if win_length is not None else n_fft
        self.ref_level = ref_level
        self.min_level = min_level

    def wav2spec(self, y):
        y = torch.stft(y, n_fft=self.n_fft,
    ...
    def spec2wav(self, spectrogram, phase):
        ...
        y = torch.istft(stft_matrix, n_fft=self.n_fft,
                        hop_length=self.hop_length,
                        win_length=self.win_length,
...
class ComplexAudio():
    ...
    def wav2spec(self, y):
        complex_spec = torch.stft(y, n_fft=self.n_fft,
                       hop_length=self.hop_length,
                       win_length=self.win_length,
                       window=torch.hann_window(self.win_length, device=y.device),
                       return_complex=True)
    ...
    def spec2wav(self, complex_spec, phase=None):
        y = torch.istft(complex_spec, n_fft=self.n_fft,
                        hop_length=self.hop_length,
                        win_length=self.win_length,
